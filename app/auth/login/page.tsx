...
// allowing the email form to show immediately instead of a blank spinner.
...
// Shown while the Privy JS chunk is downloading. Has no Privy hook dependencies so
// it renders immediately. Saves the email to sessionStorage so LoginContent can
// pick it up and auto-send the OTP once Privy finishes loading.
...
function LoginContent() {
  ...
  // Pre-populate if StaticEmailForm captured an email before Privy loaded
  ...
  // If StaticEmailForm captured an email before Privy loaded, auto-send the OTP
  // once Privy is ready. hasAutoSentRef ensures we only attempt this once.
  useEffect(() => {
    ...
    sendCode({ email: preEmail })
      ...
      .catch(() => {
        // Pre-filled email is visible; user can click Continue manually
      })
