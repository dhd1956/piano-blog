...
export function useWallet() {
    ...
    async (
      ...
      try {
        ...
        try {
          ...
          return {
            success: false,
            error: 'Contract method not found. Please check the contract address and ABI.',
          }
        }
        ...
        console.log(
          '🔧 Network test - current chainId:',
          await window.ethereum.request({ method: 'eth_chainId' })
        )
        console.log(
          '🔧 Network test - current accounts:',
        ...
        const testGas = await directContract.methods
          .submitVenue(
            venueData.name,
            venueData.city,
            venueData.contactInfo,
        ...
        const tx = await directContract.methods
          .submitVenue(
            venueData.name,
            venueData.city,
            venueData.contactInfo,
  ...
  const updateVenue = useCallback(
    async (
      ...
      try {
        ...
        const tx = await contract.methods.updateVenue(venueId, newName, newContactInfo).send({
          from: walletAddress,
          // Let MetaMask handle gas estimation and pricing
        })
  ...
  const updateVenueWithMetadata = useCallback(
    async (
      venueId: number,
      ...
      try {
        ...
        let transactionHash = ''
        const needsBlockchainUpdate =
          basicUpdates.name || basicUpdates.contactInfo || newIpfsHash !== currentVenue.ipfsHash
      ...
      try {
        ...
        if (!connectionResult) {
          ...
          return {
            success: false,
            error:
              'Wallet connection failed. Please ensure MetaMask is unlocked and on the correct network.',
          }
        }
        ...
        console.log('🔧 [verifyVenue] Transaction params:', {
          venueId,
          approved,
          from: walletAddress.substring(0, 8) + '...',
        })

        const tx = await contract.methods.verifyVenue(venueId, approved).send({
          from: walletAddress,
          // Let MetaMask handle gas estimation and pricing
        })
  ...
  const getVenueById = useCallback(
    async (venueId: number) => {
      try {
        ...
        console.log(
          '🔍 Timestamp value:',
          venue.timestamp || venue[6],
          'Type:',
          typeof (venue.timestamp || venue[6])
        )
        ...
        const hasJamSession = venue.hasJamSession !== undefined ? venue.hasJamSession : venue[8]
        const verified = venue.verified !== undefined ? venue.verified : venue[9]
        ...
        const parsedVenue = {
          ...
          submissionDate:
            Number(venue.timestamp || venue[6]) > 0
  ...
  const getVenueCount = useCallback(async (): Promise<{
    success: boolean
    count?: number
    error?: string
  }> => {
  ...
  const getCuratorNotes = useCallback(
    async (venueId: number): Promise<{ success: boolean; notes?: any; error?: string }> => {
      try {
